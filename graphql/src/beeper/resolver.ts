...
export class BeeperResolver {
  ...
  public async getAllBeepersLocation(
    @Ctx() ctx: Context,
    @Args() { latitude, longitude, radius }: BeeperLocationArgs,
  ): Promise<AnonymousBeeper[]> {
    if (radius === 0) {
      const beepers = await ctx.em.find(User, { isBeeping: true });

      return beepers.map(({ id, location }) => ({
        id: sha256(id).substring(0, 9),
    ...
    const users = await ctx.em
      ...
      .select(["id", "location"])
      .where(
        "ST_DistanceSphere(u.location, ST_MakePoint(?,?)) <= ? * 1609.34",
        [latitude, longitude, radius],
      )
      .andWhere({ isBeeping: true })
      .getResultList();

    return users.map(({ id, location }) => ({
      id: sha256(id).substring(0, 9),
      latitude: location?.latitude,
      longitude: location?.longitude,
    }));
  ...
  @Mutation(() => User)
  @Authorized()
  public async setBeeperStatus(
    @Ctx() ctx: Context,
    @Arg("input") input: BeeperSettingsInput,
  ): Promise<User> {
    if (input.isBeeping) {
      const car = await ctx.em.findOne(Car, {
        user: ctx.user.id,
        default: true,
      });
      ...
      if (!car) {
        throw new GraphQLError(
          "You need to add a car to your account to beep.",
        );
      }
    } else {
      const queueSize = await ctx.em.count(
        Beep,
        { beeper: ctx.user.id },
        { filters: ["inProgress"] },
      );
      ...
      if (queueSize > 0) {
        throw new GraphQLError(
          "You can't stop beeping when you still have riders in your queue",
        );
      }
    }
  ...
  @Mutation(() => [Beep])
  @Authorized()
  public async setBeeperQueue(
    @Ctx() ctx: Context,
    @Arg("input") input: UpdateQueueEntryInput,
  ): Promise<Beep[]> {
    ...
    const queueEntry = ctx.user.queue
      .getItems()
    ...
    const isAcceptingOrDenying =
      input.status === Status.ACCEPTED || input.status === Status.DENIED;

    const numRidersBefore = isAcceptingOrDenying
      ? ctx.user.queue
          .getItems()
          .filter(
            (entry) =>
              entry.start < queueEntry.start && entry.status === Status.WAITING,
          ).length
      : ctx.user.queue
          .getItems()
          .filter(
    ...
    if (numRidersBefore !== 0) {
      throw new GraphQLError(
        "You must respond to the rider who first joined your queue.",
      );
    }
    ...
    switch (queueEntry.status) {
      ...
      case Status.ACCEPTED:
        sendNotification({
          ...
          message:
            "You will recieve another notification when they are on their way to pick you up",
        });
        break;
    ...
    const queueNew = ctx.user.queue
      .getItems()
      .filter(
        (beep) =>
          ![Status.COMPLETE, Status.CANCELED, Status.DENIED].includes(
            beep.status,
          ),
      );
