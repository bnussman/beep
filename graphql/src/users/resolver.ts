...
export class UserResolver {
  ...
  public async getUser(
    ...
    return await ctx.em.findOneOrFail(User, id || ctx.user.id, {
      populate,
      filters: ["inProgress"],
  ...
  public async addProfilePicture(
    ...
    await s3.write(objectKey, file, { acl: "public-read" });
  ...
  @Subscription(() => User, {
    subscribe: ({ context }) =>
      Repeater.merge([context.user, pubSub.subscribe("user", context.user.id)]),
  })
  @Authorized("No Verification")
