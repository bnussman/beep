...
export class RatingResolver {
  ...
  public async rateUser(
    ...
    if (!beep) {
      throw new GraphQLError(
        "You can only leave a rating after you have been accepted",
      );
    }
    ...
    if (!user.rating) {
      ...
      const numberOfRatings = await ctx.em.count(Rating, {
        rated: input.userId,
      });

      user.rating =
        (user.rating * numberOfRatings + input.stars) / (numberOfRatings + 1);
    }
  ...
  public async getRatings(
    ...
    @Arg("filter", { nullable: true }) filter?: "recieved" | "given",
    ...
    const [ratings, count] = await ctx.em.findAndCount(
      Rating,
      {},
      {
  ...
  public async getRating(
    ...
    return await ctx.em.findOneOrFail(Rating, id, {
      populate: fieldsToRelations<Rating>(info),
    });
  }
  ...
  public async deleteRating(
    ...
    const rating = await ctx.em.findOneOrFail(Rating, id, {
      populate: ["rated"],
    });

    if (ctx.user.role === UserRole.USER && rating.rater.id !== ctx.user.id) {
      throw new GraphQLError(
        "You can't delete a rating that you didn't create.",
      );
    }
    ...
    if (numberOfRatings <= 1) {
      ...
      rating.rated.rating =
        (rating.rated.rating * numberOfRatings - rating.stars) /
        (numberOfRatings - 1);
    }
