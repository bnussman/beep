...
export class RiderResolver {
  @Mutation(() => Beep)
  @Authorized()
  public async chooseBeep(
    @Ctx() ctx: Context,
    @Arg("beeperId") beeperId: string,
    @Arg("input") input: GetBeepInput,
  ): Promise<Beep> {
    ...
    if (!beeper.isBeeping) {
      throw new GraphQLError(
        "The user you have chosen is no longer beeping at this time.",
      );
    }
  ...
  public async leaveQueue(
    ...
    const beeper = await ctx.em.findOneOrFail(User, id, {
  ...
  @Query(() => [User])
  @Authorized()
  public async getBeepers(
    @Ctx() ctx: Context,
    @Args() { latitude, longitude, radius }: GetBeepersArgs,
  ): Promise<User[]> {
    if (radius === 0) {
    ...
    const users = await ctx.em
      ...
      .orderBy({
        [raw(
          "CASE WHEN p.product_id LIKE 'top_of_beeper_list_%' THEN 1 ELSE 0 END",
        )]: QueryOrder.DESC,
        [raw(`ST_DistanceSphere(u.location, ST_MakePoint(?,?))`, [
          latitude,
          longitude,
        ])]: QueryOrder.ASC,
      })
  ...
  public async getLastBeepToRate(@Ctx() ctx: Context): Promise<Beep | null> {
    ...
    const count = await ctx.em.count(Rating, {
      rater: ctx.user.id,
    ...
    subscribe: async ({ context }) => {
      ...
      if (beep) {
        return Repeater.merge([
          beep,
          pubSub.subscribe("currentRide", context.user.id),
        ]);
      }
