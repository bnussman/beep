...
export class LocationResolver {
    ...
    if (id) {
      if (ctx.user.role !== UserRole.ADMIN) {
        throw new GraphQLError(
          "You can't update another user's location without being an admin.",
        );
      }
  ...
  public getBeeperLocationUpdates(
    @Ctx() ctx: Context,
    @Args() args: BeeperLocationArgs,
    @Root() data: AnonymousBeeper,
    @Arg("anonymize", { nullable: true, defaultValue: true })
    anonymize: boolean,
  ): AnonymousBeeper {
    if (anonymize) {
      return {
        id: sha256(data.id).substring(0, 9),
    ...
    if (ctx.user.role !== UserRole.ADMIN) {
      throw new GraphQLError(
        "You must be an admin to see un-anonamized beeper locations",
      );
    }
    return data;
  }
