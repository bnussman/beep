  ...
  return (
    ...
    >
      <Link
        href={{
          pathname: "/user/[id]",
          params: { id: beep.beeper.id, beepId: beep.id },
        }}
        asChild
      >
        <Link.Trigger>
      ...
      <View>
        ...
        {beep.status === "waiting" ? (
          <>
            ...
            <Text>
              {beep.position} {beep.position === 1 ? "person is" : "people are"}{" "}
              ahead of you.
            </Text>
            {beep.riders_waiting - 1 > 0 && (
              <Text>
                {beep.riders_waiting - 1}{" "}
      ...
      {car && (
        <View>
          ...
          <Image
            style={{ borderRadius: 10, height: 150, width: 300 }}
            resizeMode="cover"
            src={car.photo}
            alt={`car-${car.id}`}
          />
