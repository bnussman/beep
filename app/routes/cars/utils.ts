...
export const years = Array.from({ length: 50 }, (_, i) => String((new Date().getFullYear() + 1) - i));
...
export const makes = data.map(item => item.brand);

export const getModels = (make: string) => data.find(item => item.brand === make)?.models ?? [];
