...
import {
  CheckUserSubscriptionsMutation,
  PaymentsQueryQuery,
    ...
    <Card m={2}>
      <Stack space={2}>
        ...
        <SizableText fontSize="$2">
          Goes into effect immediately upon purchase
        </SizableText>
        <Image
          source={PremiumImage}
          height="300px"
...
function Package({ p }: { p: PurchasesPackage }) {
  ...
  const [checkVerificationStatus] = useMutation<CheckUserSubscriptionsMutation>(
    CheckVerificationStatus,
  );
  const { user } = useUser();

  const { data, refetch } = useQuery<PaymentsQueryQuery>(PaymentsQuery, {
  ...
  const countdown = payment?.expires ? (
    <Countdown date={new Date(payment.expires)} />
  ) : null;
  ...
  return (
    <Card p={3} py={2}>
      <XStack alignItems="center" space={2}>
        ...
        <Stack flexGrow={1} />
        {Boolean(payment) && <Check size={20} color="green" />}
        <Button
