...
declare module "rethinkdb" {
    ...
    export class Cursor {
        hasNext(): boolean;
        each(cb: (err: CursorError, row: any) => void, done?: () => void): void;
        each<T>(cb: (err: CursorError, row: T) => void, done?: () => void): void;
        each(cb: (err: CursorError, row: any) => boolean, done?: () => void): void; // returning false stops iteration
        each<T>(cb: (err: CursorError, row: T) => boolean, done?: () => void): void; // returning false stops iteration
        next(cb: (err: CursorError, row: any) => void): void;
        next<T>(cb: (err: CursorError, row: T) => void): void;
        next(): Promise<any>;
        next<T>(): Promise<T>;
        toArray(cb: (err: CursorError, rows: any[]) => void): void;
        toArray<T>(cb: (err: CursorError, rows: T[]) => void): void;
        toArray(): Promise<any[]>;
        toArray<T>(): Promise<T[]>;
        close(cb: (err: CursorError) => void): void;
        close(): Promise<void>;
    ...
    interface WriteResult {
        ...
        changes: ChangeResult[]; //used when query has { returnChanges: true }
    }
