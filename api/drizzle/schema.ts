...
export const beep = pgTable(
  ...
  (table) => {
    return {
      ...
      beeper_id_rider_id_idx: index().using(
        "btree",
        table.beeper_id,
        table.rider_id,
      ),
...
export const rating = pgTable(
  "rating",
  {
    ...
    beep_id: varchar("beep_id", { length: 255 })
      .notNull()
      .references(() => beep.id, { onUpdate: "cascade" }),
  },
  (table) => {
    return {
      rating_beep_id_rater_id_unique: unique(
        "rating_beep_id_rater_id_unique",
      ).on(table.rater_id, table.beep_id),
    };
  },
);
