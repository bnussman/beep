...
export class RiderResolver {
  ...
  public async getBeepers(@Ctx() ctx: Context, @Args(() => GetBeepersArgs) { latitude, longitude, radius }: GetBeepersArgs): Promise<User[]> {
    ...
    const users = await ctx.em.createQueryBuilder(User, 'u')
      ...
      .orderBy({
        [raw("CASE WHEN p.product_id = 'top_of_beeper_list_1_hour' or p.product_id = 'top_of_beeper_list_2_hours' or p.product_id = 'top_of_beeper_list_3_hours' THEN 1 ELSE 0 END")]: QueryOrder.DESC,
